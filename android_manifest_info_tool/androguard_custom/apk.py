...
from android_manifest_info_tool.androguard_custom import androconf
from android_manifest_info_tool.androguard_custom.dvm_permissions import DVM_PERMISSIONS
from android_manifest_info_tool.androguard_custom import bytecode
...
class APK(object):
    ...
    def __init__(self, filename, raw=False, mode="r", magic_file=None, zipmodule=ZIPMODULE):
        ...
        if zipmodule == 0:
            ...
            from android_manifest_info_tool.androguard_custom import zipfile
            ...
            if i == "AndroidManifest.xml":
                ...
                if self.xml[i] is not None:
                    ...
                    for item in self.xml[i].getElementsByTagName('permission'):
                        custom_permission = str(item.getAttribute("android:name"))
                        if item.getAttribute("android:protectionLevel") is not None \
                                and item.getAttribute("android:protectionLevel") is not '':
                            custom_permission += ":::" + str(int(item.getAttribute("android:protectionLevel"), 0))
                        else:
                            custom_permission += ":::" + "0"

                        self.customPermissions.append(custom_permission)
    ...
    def get_intent_data(self):
        ...
        for i in self.xml:
            for item in self.xml[i].getElementsByTagName('intent-filter'):
                for sitem in item.getElementsByTagName('category'):

                    ###if sitem.getAttributeNS(NS_ANDROID_URI, 'name') == category:
                    for ssitem in item.getElementsByTagName("data"):

                        host = ssitem.getAttributeNS(NS_ANDROID_URI, "host")
                        path_prefix = ssitem.getAttributeNS(NS_ANDROID_URI, "pathPrefix")
                        path_pattern = ssitem.getAttributeNS(NS_ANDROID_URI, "pathPattern")
                        scheme = ssitem.getAttributeNS(NS_ANDROID_URI, "scheme")
                        data = ''
                        if len(host) > 0:
                            data = 'host: ' + data + host + ', '
                        if len(path_pattern) > 0:
                            data += 'pathPattern: ' + path_pattern + ', '
